...
```{r}
PUP2 <- read_excel(here('datasets_regression', 'PUP2.xlsx'))
```
...
model_uni <- bind_cols(pup_uni, crude_or_co) %>% 
  select(-c(variable...9, term...10, statistic...13, p.value...14)) %>%
  rename(odds_ratio = estimate...3)
kable(model_uni)  %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
```{r}
model_multivar <- 
  glm(oc2 ~ age + gender + epigastric_pain + onset_more_24_hrs + diabetes + 
...
```{r}
model_ASA <- 
  glm(oc2 ~ factor(ASA) + age + gender + diabetes + onset_more_24_hrs, 
      family = binomial(link = 'logit'), data = PUP2)
...
```{r}
model_perf <- 
  glm(oc2 ~ degree_perforation + age + gender + diabetes + onset_more_24_hrs,
      family = binomial(link = 'logit'), data = PUP2)
...
```{r}
model_PULP <- 
  glm(oc2 ~ PULP + age + gender + diabetes + onset_more_24_hrs, 
      family = binomial(link = 'logit'), data = PUP2)
...
```{r}
model_perf2 <- 
  glm(oc2 ~ perforation + age + gender + diabetes + onset_more_24_hrs, 
      family = binomial(link = 'logit'), data = PUP2)
...
```{r}
model_hg <- 
  glm(oc2 ~ hemoglobin + age + gender + diabetes + onset_more_24_hrs, 
      family = binomial(link = 'logit'), data = PUP2)
...
```{r}
multi_confirm <- 
  bind_rows(m_ASA, m_perf, m_pulp, m_perf2, m_hg) %>% 
